...
- **Endpoint**：`GET https://app-d2y8vp5n1s75-api-79jK62Ze2pQL-gateway.appmiaoda.com/location/ip`
...
async function callIpLocation(
  ...
  const url = `https://app-d2y8vp5n1s75-api-79jK62Ze2pQL-gateway.appmiaoda.com/location/ip${
...
serve(async (req: Request): Promise<Response> => {
  ...
  if (ip) params.set("ip", ip);
  if (coor) params.set("coor", coor);
  const upstreamUrl = `https://app-d2y8vp5n1s75-api-79jK62Ze2pQL-gateway.appmiaoda.com/location/ip${
    params.toString() ? "?" + params.toString() : ""
  }`;
